...
本应用未进行 Apple 公证（个人开源项目，无 $99/年 开发者证书），macOS 首次打开可能提示"无法验证开发者"甚至"已阻止恶意软件"。**应用本身是安全的**（源码完全公开可审计），按以下步骤放行：

**方法一（推荐）：系统设置放行**
1. 双击打开 SwitchCast 被拦截后，前往 系统设置 → 隐私与安全性
2. 页面底部会出现 "已阻止 SwitchCast" 的提示，点击 **仍要打开**
3. 再次确认后即可正常使用（仅首次需要）

**方法二：终端移除隔离属性**
```bash
xattr -cr /Applications/SwitchCast.app
```
执行后直接双击打开即可。

**如果提示"已移到废纸篓"**：从废纸篓恢复 SwitchCast.app 到 Applications，然后执行方法二的命令再打开。

> 为什么会这样：macOS 对从网络下载且未经 Apple 公证的应用会标记隔离属性（quarantine）。v1.0.0 起安装包已附带 ad-hoc 签名，可通过上述方式正常放行。
